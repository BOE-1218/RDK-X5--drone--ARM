...
# 启用 core dump
ulimit -c unlimited
echo "/tmp/core.%e.%p" > /proc/sys/kernel/core_pattern 2>/dev/null || echo "cannot set core_pattern"

# 清理旧 core 文件
rm -f /tmp/core* 2>/dev/null

# 直接运行（不用 gdb），让进程崩溃产生 core dump
...
$EXEC --ros-args \
    ...
    -p fsm/flight_type:=1 \
    -p fsm/realworld_experiment:=true 2>&1
echo "EXIT_CODE=$?"
echo "---core files---"
